...
- Invoke `scripts/chrome-devtools-runner.js` inside this skill directory; a repository-root compatibility shim is not required.
...
- Use `set viewport` and `read viewport` when checking responsive layouts or mobile behavior. The runner keeps the last viewport preference across tab changes and reapplies it when needed.
